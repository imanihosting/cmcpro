...
export default function ChildminderActivityPage() {
  ...
  const fetchActivities = async () => {
    ...
    try {
      const response = await fetch(`/api/user/activity?filter=${filter}&role=childminder`);
  ...
  const filteredActivities = searchTerm.trim() === '' 
    ...
    : activities.filter(activity => 
        ...
        (activity.parent && activity.parent.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  return (
    <div>
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-600">Track your booking history, payments, and service activity</p>
      </header>
