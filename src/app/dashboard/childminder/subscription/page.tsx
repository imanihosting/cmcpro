    ...
    try {
      ...
      const response = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...
          role: 'childminder' // Specify role for the API
        }),
      });
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      if (cancelType === 'immediate') {
        ...
        await update(); // Use next-auth update method instead of fetching
  ...
  if (sessionStatus === 'loading' || isLoading) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-6">
        ...
        <p className="mt-4 text-base font-medium text-gray-700">Loading subscription information...</p>
      </div>
    );
  }

  return (
    <div className="h-full w-full bg-white p-4 sm:p-6 md:p-8">
      <div className="mx-auto max-w-5xl">
        <h1 className="mb-6 text-2xl sm:text-3xl font-bold text-gray-900">Subscription Management</h1>
        ...
        {/* Navigation tabs - Mobile-friendly design */}
          ...
          <nav className="-mb-px flex space-x-6 sm:space-x-8" aria-label="Tabs">
            <button
              ...
              className={`
                ...
                whitespace-nowrap border-b-2 px-1 py-4 text-sm sm:text-base font-medium
              `}
          ...
          <div className="space-y-6">
            <div className="overflow-hidden rounded-lg bg-gray-50 shadow-md">
              <div className="px-4 py-5 sm:p-6">
                <h3 className="text-lg sm:text-xl font-bold leading-6 text-gray-900">
                ...
                {subscription ? (
                  <div className="mt-5 border-t border-gray-200 pt-5">
                    <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                      <div className="sm:col-span-1">
                        ...
                        <dd className="mt-1 flex items-center text-sm sm:text-base text-gray-900">
                          ...
                          {subscription.cancelAtPeriodEnd && (
                            <span className="ml-2 rounded-full bg-yellow-100 px-2.5 py-1 text-xs font-medium text-yellow-800">
                    ...
                    <div className="mt-6 flex flex-col space-y-3 sm:flex-row sm:space-x-3 sm:space-y-0">
                      ...
                      {subscription.status === 'active' && !subscription.cancelAtPeriodEnd && (
                        <>
                          <button
                            onClick={() => setActiveTab('plans')}
                            className="inline-flex items-center justify-center rounded-md border border-transparent bg-blue-100 px-4 py-2 text-sm sm:text-base font-medium text-blue-700 shadow-sm hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                          >
                            Change Plan
                          </button>
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm sm:text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                          >
                            Cancel Subscription
                          </button>
                        </>
                      ...
                      {subscription.status === 'inactive' || subscription.status === 'canceled' || (
                        ...
                        <button
                          onClick={() => setActiveTab('plans')}
                          className="inline-flex items-center justify-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm sm:text-base font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                        >
                          Subscribe Again
                        </button>
                      ) : null}
                  ...
                  <div className="mt-5 text-center">
                    <p className="text-sm sm:text-base text-gray-600">You don't have an active subscription.</p>
                    <button
                      onClick={() => setActiveTab('plans')}
                      className="mt-4 inline-flex items-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm sm:text-base font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                    >
        ...
        {activeTab === 'plans' && (
          <div className="space-y-6">
            {/* Billing interval toggle - Mobile-friendly */}
            <div className="flex justify-center">
              <div className="relative mt-2 rounded-md shadow-sm">
                <div className="inline-flex rounded-md shadow-sm">
                  <button
                    type="button"
                    className={`relative inline-flex items-center rounded-l-md px-3 py-2 text-sm sm:text-base font-medium ${
                  ...
                  <button
                    type="button"
                    className={`relative -ml-px inline-flex items-center rounded-r-md px-3 py-2 text-sm sm:text-base font-medium ${
            ...
            {/* Plans grid - Mobile-first responsive design */}
                ...
                return (
                  <div 
                    ...
                    className={`overflow-hidden rounded-lg border ${
                      plan.isPopular ? 'border-blue-400 ring-2 ring-blue-500' : 'border-gray-200'
                    } bg-white shadow-sm transition-shadow hover:shadow-md`}
                  >
                    {plan.isPopular && (
                      <div className="bg-blue-500 py-1 text-center text-xs sm:text-sm font-medium uppercase tracking-wider text-white">
                        Most Popular
                      </div>
                    )}
                    
                    <div className="p-5 sm:p-6">
                      <h3 className="text-lg sm:text-xl font-bold leading-6 text-gray-900">{plan.name}</h3>
                      
                      <p className="mt-4 flex items-baseline">
                        <span className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">
                          {priceToShow.formattedPrice}
                        </span>
                        <span className="ml-1 text-sm sm:text-base font-medium text-gray-500">
                      ...
                      <div className="mt-6">
                        {isCurrentPlan ? (
                          <button
                            disabled
                            className="w-full rounded-md bg-gray-100 px-4 py-3 text-sm sm:text-base font-medium text-gray-500"
                          >
                            Current Plan
                          </button>
                        ) : (
                          <button
                            ...
                            disabled={isUpdating}
                            className="w-full rounded-md bg-blue-600 px-4 py-3 text-sm sm:text-base font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                          >
                            {isUpdating ? (
                              <>
        ...
        {/* Cancellation confirmation modal - Mobile-friendly */}
        {showCancelConfirm && (
                ...
                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      <h3 className="text-lg sm:text-xl font-bold leading-6 text-gray-900">Cancel Subscription</h3>
                      <div className="mt-2">
                        ...
                        <div className="mt-4 space-y-3">
                          <div className="flex items-center">
                            ...
                            />
                            <label htmlFor="cancel-end" className="ml-2 block text-sm sm:text-base text-gray-700">
                          ...
                          <div className="flex items-center">
                            <input
                              ...
                              className="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500"
                            />
                            <label htmlFor="cancel-immediate" className="ml-2 block text-sm sm:text-base text-gray-700">
